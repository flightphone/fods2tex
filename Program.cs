...
namespace fods2tex
{
    ...
    class Program
    {
        //https://kub-24.ru/putevoj-list-avtomobilya/
        ...
        static void initStyles(XDocument xmdoc)
        {
            ...
            var parcl = xmdoc.Descendants(style + "style").Where(u => (u.Attribute(style + "family").Value == "paragraph") ||
        ...
        static string GetText(XElement tcel, bool multi = false)
        {
            ...
            {
                tag = tcel.Attribute(text + "style-name").Value;
            }
            if (tcel.Attribute(table + "style-name") != null)
            {
                tag = tcel.Attribute(table + "style-name").Value;
            }
            if (!string.IsNullOrEmpty(tag))
                if (styles.ContainsKey(tag))
                {
                    ...
                    if (st.IndexOf("\\textbf{") > -1 || st.IndexOf("\\textit{") > -1)
                        multi = true;
        ...
        static void Main(string[] args)
        {
            ...
            if (string.IsNullOrEmpty(gm))
                gm = "\n\\usepackage[paperheight=29.7cm,paperwidth=21.00cm,left=1.27cm,right=1.27cm,top=0.751cm,bottom=0.751cm]{geometry}";
            ...
            foreach (XElement tab in tables)
            {
                ...
                int stopcol = 1024;
                if (tab.Attribute("stop") != null)
                {
                    stopcol = int.Parse(tab.Attribute("stop").Value);
                }
                ...
                foreach (XElement trow in tab.Elements(table + "table-row"))
                {
                    ...
                    foreach (XElement tcel in trow.Elements())
                    {
                        ...
                        string snn = (tcel.Attribute(table + "number-columns-repeated") ?? repe).Value;
                        int nnn = int.Parse(snn);

                        //Все повторяем nnn раз 
                        if (ipos + nnn >= ncol)
                            nnn = 1;
                        for (int iii = 0; iii < nnn; iii++)
                        {
                            ...
                            {
                                if (tcel.Attribute(table + "number-rows-spanned").Value != "1")
                                {
